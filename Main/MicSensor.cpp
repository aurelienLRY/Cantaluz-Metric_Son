...
void micUpdatePeakSmooth(int peak) {
  float factor = g.run.peakSmoothFactor;
  if ((float)peak < g.peakSmooth) {
    factor *= 4.0f;
    if (factor > 0.75f) factor = 0.75f;
  }
  g.peakSmooth += ((float)peak - g.peakSmooth) * factor;
}
